...
const analyticsController = {
    ...
    async getPriceAnalytics(req, res, next) {
        try {
            ...
            const priceHistory = await analyticsService.getPriceHistory(productId, startDate ? new Date(startDate) : null, endDate ? new Date(endDate) : null);
            const analysis = {
    ...
    async getSpendingAnalysis(req, res, next) {
        try {
            ...
            const analysis = await analyticsService.getSpendingAnalysis(req.user.uid, new Date(startDate), new Date(endDate));
    ...
    async getVendorAnalysis(req, res, next) {
        try {
            ...
            const comparison = await analyticsService.getVendorComparison(productIds ? productIds.split(',') : []);
            const performance = await analyticsService.calculateVendorPerformance(vendorIds ? vendorIds.split(',') : []);
    ...
    async getCategoryAnalysis(req, res, next) {
        try {
            ...
            const { startDate, endDate } = req.query;
            const analysis = await analyticsService.getCategoryAnalysis(req.user.uid, categoryId, startDate ? new Date(startDate) : null, endDate ? new Date(endDate) : null);
            const performance = {
    ...
    async getPriceComparisonReport(req, res, next) {
        try {
            ...
            const comparisons = await Promise.all(productIdArray.map(async (productId) => {
                const elasticity = await advancedAnalyticsService.analyzePriceElasticity(req.user.uid, productId, parseInt(timeframe));
