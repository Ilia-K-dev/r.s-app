...
class DocumentClassifier {
    ...
    _analyzeStructure(blocks) {
        ...
        const lineSpacings = blocks.map(block => block.boundingBox?.top - (block.boundingBox?.bottom || 0));
        const avgSpacing = lineSpacings.reduce((a, b) => a + b, 0) / lineSpacings.length;
        const spacingConsistency = lineSpacings.every(spacing => Math.abs(spacing - avgSpacing) < avgSpacing * 0.2);
        ...
        const alignmentConsistency = leftAlignments.every(align => Math.abs(align - avgLeftAlign) < 20);
        if (alignmentConsistency)
            structureScore += 0.3;
        // Check for consistent font sizes
        const hasConsistentFontSize = blocks.every((block, i, arr) => i === 0 || block.boundingBox?.height === arr[i - 1].boundingBox?.height);
        if (hasConsistentFontSize)
            structureScore += 0.4;
    ...
    _assessTextQuality(blocks) {
        ...
        const commonErrors = blocks.some(block => block.text.includes('?') || block.text.includes('�'));
        if (!commonErrors)
            qualityScore += 0.3;
        return {
            score: qualityScore,
