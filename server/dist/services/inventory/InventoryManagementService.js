...
class InventoryManagementService {
    ...
    async adjustStockLevels(productId, adjustments) {
        try {
            ...
            const alerts = await this._checkStockLevels({ ...product.data(), ...adjustments }, product.data().currentStock);
            if (alerts.length > 0) {
                await this._createAlerts(alerts, productId);
            }
            return {
    ...
    _calculateTurnoverRate(product) {
        ...
        const averageInventoryValue = ((product.currentStock * product.unitPrice) +
