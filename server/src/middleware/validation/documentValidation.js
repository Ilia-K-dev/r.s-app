      ...
      await Promise.all(files.map(async (file) => {
        if (file.mimetype.startsWith('image/')) {
          ...
          if (metadata.width < MIN_IMAGE_DIMENSIONS.width || 
            ...
            throw new AppError(
              `Image dimensions too small. Minimum dimensions are ${MIN_IMAGE_DIMENSIONS.width}x${MIN_IMAGE_DIMENSIONS.height}. ` +
              `Provided dimensions: ${metadata.width}x${metadata.height}`,
              400
            );
          }
          ...
          if (metadata.width > MAX_IMAGE_DIMENSIONS.width || 
            ...
            throw new AppError(
              `Image dimensions too large. Maximum dimensions are ${MAX_IMAGE_DIMENSIONS.width}x${MAX_IMAGE_DIMENSIONS.height}. ` +
              `Provided dimensions: ${metadata.width}x${metadata.height}`,
              400
            );
          }
      ...
      if (options.resize) {
        ...
        if (width && (width < MIN_IMAGE_DIMENSIONS.width || width > MAX_IMAGE_DIMENSIONS.width)) {
          validationErrors.push(
            `Invalid resize width. Must be between ${MIN_IMAGE_DIMENSIONS.width} and ${MAX_IMAGE_DIMENSIONS.width}`
          );
        }

        if (height && (height < MIN_IMAGE_DIMENSIONS.height || height > MAX_IMAGE_DIMENSIONS.height)) {
          validationErrors.push(
            `Invalid resize height. Must be between ${MIN_IMAGE_DIMENSIONS.height} and ${MAX_IMAGE_DIMENSIONS.height}`
          );
        }
      }
  ...
  static validateBatchProcessing(req, res, next) {
    try {
      ...
      if (totalSize > maxBatchSize) {
        throw new AppError(
          `Total batch size too large. Maximum is ${maxBatchSize / 1024 / 1024}MB. ` +
          `Current batch size: ${(totalSize / 1024 / 1024).toFixed(2)}MB`,
          400
        );
      }
