...
class ReceiptProcessingService {
  ...
  _extractItems(lines) {
    ...
    for (const line of lines) {
      if (this._isNonItemLine(line)) continue;
  ...
  _validateAndClean(receiptData) {
    ...
    if (receiptData.totals.total === null) {
      receiptData.totals.total = receiptData.items.reduce(
    ...
    try {
      ...
      if (searchParams.text) {
        ...
        return snapshot.docs
          ...
          .filter(receipt => 
            receipt.merchant?.toLowerCase().includes(searchText) ||
