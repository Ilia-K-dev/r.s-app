...
class StockMovement {
  ...
  validate() {
    ...
    if (!this.type) errors.push('Movement type is required');
    if (!['add', 'subtract', 'adjust', 'transfer'].includes(this.type)) {
      errors.push('Invalid movement type');
    }
    if (typeof this.quantity !== 'number') errors.push('Quantity must be a number');
    if (this.quantity <= 0) errors.push('Quantity must be positive');
    if (typeof this.previousStock !== 'number') errors.push('Previous stock must be a number');
    if (typeof this.newStock !== 'number') errors.push('New stock must be a number');
    if (this.newStock < 0) errors.push('New stock cannot be negative');
